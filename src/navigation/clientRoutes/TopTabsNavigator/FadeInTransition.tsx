...
import { WIDTH } from '../../../utils/device.ts';
...
const FadeInTransition = ({ index = 0, children, direction, animate, containerStyle }: FadeInTransitionProps) => {
  const progress = useSharedValue(0);

  useEffect(() => {
    if (animate) {
      progress.value = withDelay(index * 75, withSpring(1, { damping: 80, stiffness: 200 }));
    } else {
      progress.value = 0;
    }
  }, [animate]);
